...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
    ...
    return (
        <div className="relative w-full h-full overflow-hidden rounded-[34px] border border-white/10 bg-white/[0.02] shadow-[0_24px_60px_rgba(2,8,23,0.24)] backdrop-blur">
            {/* The WebGL Canvas */}
            <canvas
                id="infinite-grid-menu-canvas"
                ref={canvasRef}
                className="cursor-grab w-full h-full outline-none active:cursor-grabbing"
            />

            {/* Left Scrim */}
            <div className={`absolute inset-y-0 left-0 w-full md:w-[60%] lg:w-[45%] pointer-events-none bg-gradient-to-r from-black/85 via-black/45 to-transparent transition-opacity duration-300 ${isMoving ? 'opacity-0' : 'opacity-100'}`} />
            ...
            <AnimatePresence>
                {activeItem && !isMoving && (
                    <motion.div
                        ...
                        className="absolute inset-y-0 left-0 z-10 flex flex-col justify-between p-8 md:p-10 w-full md:max-w-[420px] pointer-events-none"
                    >
                        {/* Top text content */}
                        <div className="space-y-4 pointer-events-auto">
                            <div className="flex items-center gap-3">
                                {activeItem.status && (
                                    <span className={`inline-flex items-center rounded-full bg-gradient-to-r ${activeItem.accent || 'from-amber-300 to-orange-500'} px-2.5 py-0.5 text-xs font-semibold text-amber-950 shadow-md`}>
                                        {activeItem.status}
                                    </span>
                                )}
                                <span className="text-[10px] tracking-[0.2em] text-slate-300/80 uppercase font-mono bg-white/5 border border-white/10 px-2 py-0.5 rounded">
                                    Project Intel
                                </span>
                            </div>

                            <h3 className="font-display text-3xl md:text-4xl font-bold tracking-tight text-white leading-tight drop-shadow-[0_4px_12px_rgba(0,0,0,0.6)]">
                                {activeItem.title}
                            </h3>

                            <p className="text-sm md:text-base leading-relaxed text-slate-300 max-w-[32ch] drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)]">
                                {activeItem.description}
                            </p>
                        </div>

                        {/* Bottom stack + action button */}
                        <div className="space-y-5 pt-4 pointer-events-auto">
                            {activeItem.stack && activeItem.stack.length > 0 && (
                                <div className="space-y-2">
                                    ...
                                    <div className="flex flex-wrap gap-1.5 max-w-[32ch]">
                                        {activeItem.stack.map((tech) => (
                                            <span 
                                                key={tech} 
                                                className="inline-flex items-center rounded-md bg-black/40 border border-white/10 px-2 py-0.5 text-xs text-slate-200 backdrop-blur-sm transition hover:bg-black/60"
                                            >
                                                {tech}
                                            </span>
                                        ))}
                            ...
                            <button
                                ...
                                className="group relative w-full inline-flex h-11 items-center justify-center overflow-hidden rounded-xl bg-gradient-to-r from-amber-200 to-amber-100 text-amber-950 font-bold transition-all hover:brightness-110 active:scale-[0.98] shadow-lg shadow-black/20"
                            >
                                <span className="mr-1.5 text-sm select-none">Investigate Project</span>
                                <span className="text-lg select-none transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5">&#x2197;</span>
                            </button>
                        </div>
                    </motion.div>
                )}
            ...
            {/* Dots Pagination */}
            {items.length > 0 && (
                <div className={`absolute bottom-6 left-1/2 -translate-x-1/2 z-10 flex gap-2 transition-opacity duration-300 ${isMoving ? 'opacity-30' : 'opacity-100'}`}>
                    {items.map((_, i) => {
                        const isActive = activeItem ? items.indexOf(activeItem) === i : false;
                        return (
                            <div
                                key={i}
